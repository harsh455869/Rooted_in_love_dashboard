...
function OrderInfo(props) {
  ...
  const saveOrderStatus = () => {
    ...
    SetLoading(true)
    axios
      .post(`${config.serverURL}admin/order/status/update`, data)
      .then(() => {
        toast("Status Updated", {
          position: "bottom-center",
          type: "success",
  ...
  useEffect(() => {
    if (!props.addnew) {
      setOrderData(categoryData); // Set initial order data if not adding a new order
    } else {
      setOrderData({}); // Reset for new order
    }
        ...
        <div className="formadj">
          <div className="row">
            <div className="col-12">
              ...
              {
                loading &&  <Loader type="spinner-circle" bgColor="black" color="black" size={100} />
              }
              <AppBar position="static">
                <Tabs value={value} onChange={handleTabs}>
                  <Tab label="Order Details" />
              ...
              <TabPanel value={value} index={0}>
                ...
                <div style={{ justifyContent: "flex-end" }} className="row">
                  <div className="col-5">
                    <p style={{ fontSize: 16, lineHeight: "17px" }}>
                      <span style={{ fontSize: 18, fontWeight: "bold" }}>Total Amount</span> - Rs.{orderData?.total}
                    </p>
                    <p style={{ fontSize: 12 }}>Payment Id: {orderData?.payment_id}</p>
                  </div>
                </div>
                <div className="col-5">
                  ...
                  <select
                    ...
                    name="enabled"
                  >
                    {['dispatched', 'delivered', 'cancelled'].map((option) => (
                ...
                <div className="row">
                  <div className="col-5">
                    <p style={{ fontSize: 20 }}>Customer Info</p>
                    <p style={{ fontWeight: "400", fontSize: "16px", marginTop: "0px" }}>
                      Customer Name - {orderData?.user_name}
                    </p>
                    <p style={{ fontWeight: "400", fontSize: "16px", marginTop: "0px" }}>
                      Mobile Number - {orderData?.user_phoneno}
                ...
                <div className="row">
                  <div className="col-12">
                    <button className="btn1" onClick={saveOrderStatus}>
                      Save
                    </button>
                  </div>
                </div>
