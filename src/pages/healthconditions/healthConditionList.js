...
function HealthConditionList() {
    ...
    function getData(pageNumber = 1) {
        ...
        axios.get(`${config.serverURL}admin/gethealthconditions`)
            .then((res) => {
                    ...
                    <div className="col-lg-11 col-10">
                        <div className="row upperhead">
                            ...
                            <div className="col-12" style={{ textAlign: 'right', display: 'block' }}>
                                ...
                                <button type="button" className="btn btn-primary" style={{ display: 'inline-block', margin: 'unset', marginTop: '50px', marginRight: '30px', background: 'white', fontWeight: 600, color: '#254c86', border: 'solid 2px #254c86' }} onClick={() => { setisaddnew(true); setmodalOpened4(true) }}>       Add health condition  Related Product      </button>
                            </div>
                        </div>
                        <div className="row upperhead">
                            ...
                            <div className="col-lg-12 col-12 setheight2">
                                <table className="table">
                                    <thead>
                                        <tr>
                                            <th>NAME</th>
                                            <th>Recommanded Product</th>
                                            <th>Hot selling product</th>
                                        </tr>
                                    </thead>
                                            ...
                                            data.filter(item =>
                                                ...
                                                return (
                                                    <tr key={index}>
                                                        ...
                                                        <td>
                                                            {item.recommended_product.map((product, idx) => (
                                                                <div key={idx}>{product.name}</div> // Displaying each recommended product
                                                            ))}
                                                        </td>
                                                        <td>
                                                            {item.hot_selling_product.map((product, idx) => (
                                                                <div key={idx}>{product.name}</div> // Displaying each hot selling product
                                                            ))}
                                                        </td>
                                                    </tr>
                                                );
